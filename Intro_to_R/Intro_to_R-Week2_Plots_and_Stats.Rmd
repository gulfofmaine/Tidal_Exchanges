...
author: "LGCarlson, M.Barajas, A.Hart"
date: "6/1/2020"
...
# Load package containign pdenguine data
...
`hist()` - This base R function computes a histogram of the given data
values. The argument must be a numeric vector (a column of data is a
vector).
...
For this, you'll need the ggplot2 package which is loaded as part of the
tidyverse.
...
We could also hypothesize that sexual dimorphism would cause variation
is mass between the sexes (i.e. males are typically larger).
...
And a more advanced version that is color-coded by species (`ggplot`
will automatically assign colors, but you can manually change them as
well):
...
simple linear model ("lm") is appropriate
...
### More plotting features to know

There are lots of great packages and tools to add nice plot color
palettes, including those that are colorblind friendly. Some of our
favorites include:

-   [ColorBrewer](https://colorbrewer2.org/#type=sequential&scheme=BuGn&n=3)
    has lots of options for high-contrast and colorblind friendly color
    palettes

-   [`viridis`](https://cran.r-project.org/web/packages/viridis/vignettes/intro-to-viridis.html)
    is a nice package with colorblind friendly color palettes that are
    easy to use for both continuous and discrete data

-   There are lots of cool tools like the
    [`imgpalr`](https://github.com/leonawicz/imgpalr) package that make
    it easy to pull colors for a custom color palette (can be good for
    presentations), but if you go this route always be sure to check
    that your plot features show up in grayscale and are colorblind
    friendly (the [`cvdPlot()` function from the
    `colorBlindness`](https://cran.r-project.org/web/packages/colorBlindness/vignettes/colorBlindness.html)
    package is a nice way to check quickly).

-   Just for fun, there are pre-made color palettes for many of Wes
    Anderson's movies in the
    [`wesanderson`](https://github.com/karthik/wesanderson) package.

## Principal component analysis (PCA)

-   Exploratory data analysis
-   Dimension reduction while preserving information
-   Represent the directions of the data that explain the most variance
-   "new axes that provide the best angle to see and evaluate the data"

```{r}

# Characterize penguin species differences and similarities

# select numeric variables of interest and scale them
# scale() default is to center and scale data (subtract mean and divide by SD)
# puts variables in same range for analysis
penguins_complete %>%
  select(bill_length_mm,bill_depth_mm,flipper_length_mm,body_mass_g)%>%
  scale()->peng

# column names
colnames(peng)<-c("bill_length_mm","bill_depth_mm","flipper_length_mm","body_mass_g")

# Principal Component Analysis
peng_pca<-prcomp(peng)

# summary output
summary(peng_pca)
# principal component 1 explains 0.6863 (68.63%) variance
# principal component 2 explains 0.1945 (19.45%) variance

# Rotation are called "loadings"
# 4 principal components
# there are as many principal components as there are variables in the data
peng_pca
# they are the linear combination of weights for each variable that contribute to that "component"
...
# species names
peng_species<-penguins_complete$species

# variable loadings
var<-as_tibble(peng_pca$rotation)

# variable names
varnames<-row.names(peng_pca$rotation)

# individual scores
ind <- as.data.frame(peng_pca$x)

# plot of PC1 and PC2 scores and variable loadings
ggplot()+
  geom_point(data=ind, aes(x=PC1,PC2,color=peng_species))+
  geom_segment(data=var,aes(xend=PC1,yend=PC2),x=0,y=0,
               arrow=arrow(length=unit(.05,"inches"),type="closed"))+
  geom_text(data=var,aes(x=PC1,y=PC2),label=varnames,
            hjust=0,vjust=1,size=4)
```

-   PC1 (x-axis) associated with flipper length and body mass
-   PC2 (y-axis) associated with bill size variables
-   Gentoo characterized by longer flippers and larger body mass
-   Gentoo more different than other species in terms of flippers and
    body mass
-   Adelie and Chinstrap more similar to each other (smaller with
    shorter flippers)
-   Adelie and Chinstrap differentiated by bill shape
-   Chinstrip bill length longer than Adelie (further down)
-   Adelie bills deeper than Gentoo and are maybe slightly deeper than
    Chinstrap (furthest to the left)

```{r}

# scatter plots to confirm

ggplot(penguins_complete, aes(x = flipper_length_mm, y = body_mass_g, colour = species)) +
  geom_point()

ggplot(penguins_complete, aes(x = bill_depth_mm, y = bill_length_mm, colour = species)) +
  geom_point()
...
**A representation for a purpose:**
...
Distributional tests help test the hypothesis that your data was sampled
from the hypothesized distribution (here the hypothesized distribution
is the model).

Statistical models describe relationships between the data used to build
them and can be used to evaluate hypotheses about these relationships
(e.g. does a relationship exist, does it have a directional trend,
ect.). These models are a special type of mathematical model which are
...
1.  Explanation -- we care about understanding the form of our model, in
    ...
    explanatory variables or better understanding the processes that
...
-   Distribution - usually refers to the statistical shape of the data
    (e.g. normal/Gaussian, Poisson, ect.)
...
I used a density ridgeplot rather than a histogram, because the
...
# Used to determine if two categorical variables have significant relationship
...
penguins_complete%>%
  ...
  geom_jitter() # moves points a little so we can see them better (try excluding this line to see what happens)

# table
table(penguins_complete$island,penguins_complete$species)
# input data is in form of table

# Chi-Square test
chisq.test(table(penguins_complete$island,penguins_complete$species))
# check the p-value, strong relationship

# sex and species
penguins_complete%>%
  filter(!is.na(sex))%>%
  ggplot(aes(x=sex,y=species,color=species))+
  geom_jitter()

# table
table(penguins_complete$sex,penguins_complete$species)

# test
chisq.test(table(penguins_complete$sex,penguins_complete$species))
# no meaningful relationship

# island and sex
penguins_complete%>%
  filter(!is.na(sex))%>%
  ggplot(aes(x=island,y=sex,color=sex))+
  geom_jitter()

# table
table(penguins_complete$island,penguins_complete$sex)

# test
chisq.test(table(penguins_complete$island,penguins_complete$sex))
# no meaningful relationship
...
## Models to describe relationships: linear models, GLMs, GAMs

### Steps of model design:

1.  Describe, wrangle & visualize dataset - often this is part of
    hypothesis development
2.  Specify response and explanatory variables of interest
3.  Select a model architecture i.e., linear model, generalized linear
    model (GLM), generalized additive model (GAM), ect.
4.  Fit/train a model using data
5.  Evaluate the model's performance (e.g. predictive power, deviance
    explained, compare to other models, ect.)
...
Hypothesis: Weight and flipper length are related so we can use flipper
length to predict body mass of Adelie penguins that we couldn't weigh
today.
...
-   Data frame -- collection of variables Function -- tool used to build
    and evaluate models Formula -- a way to describe how you want to
    relate variables to each other

-   A formula contains the response variable seperated by a tilde \~
    from the explanatory variable, for example:
...
ggplot() + 
  ...
  geom_point(data=mass_flipper_preds2, aes(x=flipper_length_mm, y=predicted_mass, color = sex), pch=17, size=2) + # triangles are predicted mass, round points are observations for birds we could weigh
...
GLMs relax the assumption that responses are normally distributed, an
assumption that biological data often violates.

We can implement the same model to relate body weight to flipper length
as a GLM:

```{r}
GLM1 <- glm(body_mass_g ~ flipper_length_mm + 0, family = gaussian, data = adelie_penguins)

GLM1
```

These results look very similar to the linear model because we assumed a
...
To change this assumption we can change the `family` argument

```{r}
GLM2 <- glm(body_mass_g ~ flipper_length_mm + 0, family = poisson, data = adelie_penguins)

GLM2
```

To select between models we may compare:

-   AIC - balances fit with model parsimony (# of parameters), smaller
    is better, scores within +/-2 of each other are equivalent

-   Null deviance - how well response is predicted by just the intercept

-   Residual deviance - how well response predicted by intercept &
    predictor variables

-   You can find a nice description of deviance
    [here](https://bookdown.org/egarpor/PM-UC3M/glm-deviance.html).

GLM2 model with a Poisson distribution assumption has a larger AIC so
our first diagnostic suggests that we should stick with a gaussian
assumption.

When we look at the deviance explained for both models, the GLM2
explains more deviance.

```{r}
# Calculate the deviance explained by the models
dev.explained1 <- (GLM1$null.deviance - GLM1$deviance)/GLM1$null.deviance

dev.explained2 <- (GLM2$null.deviance - GLM2$deviance)/GLM2$null.deviance

# Round to 3 decimal places
dev.explained1 <- round(dev.explained1,3)
dev.explained2 <- round(dev.explained2,3)

dev.explained1
dev.explained2
```

For Poisson models we may also want to check for overdispersion (if
there is overdispersion, this isn't an appropriate distribution to
assume):
...
The p-value is very small for this dispersion test, indicating that
there is overdispersion so this model is not appropriate for our data.

### Generalized Additive Models (GAMs)

GAMs provide another level of flexibility by allowing for nonlinear
relationships between variables. Both GLMs and GAMs can also account for
...
GAM1 <- gam(body_mass_g ~ flipper_length_mm + 0, family = gaussian, data = adelie_penguins)

GAM1
```

Now let's see if including categorical sex variables improves our GAM
prediction.

```{r}
GAM2 <- gam(body_mass_g ~ flipper_length_mm + sex + 0, family = gaussian, data = adelie_penguins)

GAM2
```

To compare GAMs you may look at:

-   Generalised cross-validation (GCV) score - smaller values indicate
    better fit, GCV used to select model smoothness

-   Deviance explained - larger is generally better

-   Significance for model predictor variables

GAM2 that includes flipper length and sex as predictor variables have a
smaller GCV score, suggesting a better fit.

Let's take a look at some of these other diagnostics for both GAMs:
...
Flipper length was a significant variable in both models so it supports
our hypothesis that this is a good predictor of body weight. GAM2 has a
higher deviance explained so sex does appear to help explain body weight
but not as significantly as flipper length.

We covered the basics of GLMs and GAMs here but there are much more
extensive tutorials that provide more specifics, e.g.:

-   Intro to GLMs & GAMs:
    <https://christophm.github.io/interpretable-ml-book/extend-lm.html>

-   More detailed GLM examples:
    <https://timnewbold.github.io/teaching_resources/GLMs.html>

### Other things to think about

-   It is generally best to avoid including multiple explanatory
    variables that are highly correlated (e.g. bottom temperature and
    depth are rarely included in models together since they are
    correlated variables)

-   You can include known interactions in models (e.g. latitude and
    longitude are often reported together for ecological sampling, if
    you want to include the effect of sample location in a model it is
    often as an interaction between these two variables), one way to do
    this is via
    [splines](https://fromthebottomoftheheap.net/2014/05/09/modelling-seasonal-data-with-gam/):

```{r, eval=FALSE}
GAMExample <- gam(sample ~ s(LONGITUDE, LATITUDE) + DEPTH)
# this code won't run since we didn't provide any real data, but assumes that a  hypothetical data table contains sample, LONGITUDE, LATITUDE, and DEPTH columns
```
...
# After running you will see a "cnty" object in your environment
...
1.  Maps often contain multiple layers of data (e.g. your data,
    ...
    -   We often provide data to the `data` argument of **each map
        layer** and specify separate aesthetics using the `aes()`
        argument rather than setting up global plot options by providing
        this information directly to `ggplot()`
...
3.  Over a large enough area we need to account for the curvature of the
    earth (i.e. pick an appropriate cartographic projection)
...
am going to add a lower-resolution coastline next that will cover up the
white.
...
# Be careful to ensure that your projected coordinates match across plots otherwise you may plot data outside of your plot window!!!
...
Now add these layers to the plot to show political boundaries on land:
...
Now that our map shows major geographic features, we can add point data
relevant to our own work (here we will add data points from the Gulf of
Maine longline survey).

The `ecodata` package is maintained by the Northeast Fishery Science
Center's Ecosystem Dynamics branch and contains much of the data used in
their annual State of the Ecosystem Report, including data like shape
files for ecological production units (`ecodata::epu_sf`).

```{r}
# remotes::install_github("noaa-edab/ecodata",build_vignettes=TRUE)
library(ecodata) 
GOMlongline <- ecodata::long_line_survey # Load longline survey data

# Let's assume that we only care about survey stations that observed blackbelly rosefish, we can select only those data
rosefish <- GOMlongline %>%
        filter(COMMON_NAME == "BLACKBELLY ROSEFISH")
```

Now we can add the survey stations that observed blackbelly rosefish,
but let's color stations by season so it is easy to see where fish were
observed in the spring vs. fall.
...
ggplot() +
  ...
  geom_point(data = rosefish, aes(x=DECDEG_BEGLON_SET, y=DECDEG_BEGLAT_SET, col = SEASON)) + # Add blackbelly rosefish survey stations using the latitude/longitude at the beginning of the survey set to add points
        
  coord_sf(xlim = c(-74, -64),  ylim = c(40, 47.3)) + 
  theme_minimal() 
```

Now we have points at all of our survey locations, but the colors are
difficult to see and many points overlap. We can change the shape rather
than color to distinguish between season and make points
semi-transparent to see them better.
...
ggplot() +
  ...
  geom_polygon(data = Canada, color = "black", fill = "gray", aes(x=long, y=lat, group=group)) + 
  geom_sf(data = US_trans, color="black", fill = "gray") + 
  geom_point(data = rosefish, aes(x=DECDEG_BEGLON_SET, y=DECDEG_BEGLAT_SET, shape=SEASON),alpha = 0.5) + # shape changes the point shape, alpha changes the transparency
...
```{r}
 ggplot() +
  ...
  geom_polygon(data = Canada, color = "black", fill = "gray", aes(x=long, y=lat, group=group)) + 
  geom_sf(data = US_trans, color="black", fill = "gray") +
  geom_point(data = rosefish, aes(x=DECDEG_BEGLON_SET, y=DECDEG_BEGLAT_SET, shape=SEASON),alpha = 0.5) +
  coord_sf(xlim = c(-74, -64),  ylim = c(40, 47.3)) +
  theme_minimal() + 
  ...
  annotate("text", x = -69.5, y = 43.35, label = "Gulf of Maine", hjust=0) +
...
-   Don't spend lots of time fiddling with placement of different
    elements (e.g. annotations, inset maps, scale bars, ect.) in your
    plot window! First specify dimensions for the saved plot and check
    the saved version as you make adjustments.
